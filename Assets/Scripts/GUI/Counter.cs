...
public class Counter : MonoBehaviour {
  //  UILabel label;
    ...
    string GetTarget()
    {
        ...
        {
            if( InitScript.Instance.currentTarget == Target.Top ) return "消除所有泡泡";
            else if( InitScript.Instance.currentTarget == Target.Chicken ) return "营救小鸡仔";

        }
        else
        {
            if( LevelData.mode == ModeGame.Vertical ) return "消除所有泡泡";
            else if( LevelData.mode == ModeGame.Rounded ) return "营救小鸡仔";
